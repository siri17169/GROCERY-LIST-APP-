        ...
        return Hero(
          ...
          child: GestureDetector(
            onTap: () => Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => CatDetail(name: cat['n'], items: filtered, color: cat['c'])),
            ),
            child: Container(
...
class CatDetail extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: items.isEmpty
          ? const Center(child: Text("Empty Category"))
          : ListView.builder(
              padding: const EdgeInsets.all(16),
              itemCount: items.length,
              itemBuilder: (context, i) => Card(
                child: ListTile(leading: Text(items[i].emoji), title: Text(items[i].name)),
              ),
            ),
    );
  }
